...
namespace AddInsShare.Schema
{
  internal class CustomType : Type
  {
    ...
    private          IList<CustomPropertyInfoHelper> _customProperties;

    public CustomType(Type delegatingType, IList<CustomPropertyInfoHelper> customProperties)
    {
    ...
    {
      return _baseType.GetProperties(bindingAttr).Concat(_customProperties).ToArray();
    }
    ...
    protected override PropertyInfo GetPropertyImpl(string name, BindingFlags bindingAttr, Binder binder, Type returnType, Type[] types, ParameterModifier[] modifiers)
    {
      ...
      return GetProperties(bindingAttr).FirstOrDefault(prop => prop.Name == name)
        // and then for a custom property
          ?? _customProperties.FirstOrDefault(prop => prop.Name == name);
    }
